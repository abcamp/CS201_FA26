...
- How many rows and columns are in the `mtcars` dataframe?

  - 32 rows (observations) & 12 columns (variables)
...
# you need to select the specific variable
...
median(mtcars$mpg) # see how easy? :)
...
# histogram
ggplot(data = mtcars, aes(x = drat)) + # x only, no y because y is automatically "count"
  geom_histogram()
...
# am is currently numeric--not so helpful for our boxplot
# needs to be factor because it's a categorical variable
# convert am to factor by creating new am_f variable
mtcars <- mtcars |> 
  mutate(am_f = factor(am, labels = c("auto", "manual")))

class(mtcars$am_f) # check the class of our new variable

ggplot(data = mtcars, aes(x = am_f, y = mpg)) +
  geom_boxplot()
